...
export class ManagerService {
  ...
  getManagers(): Observable<ApiResponse<ManagerResponse[]>> {
    return this.http
      .get<ApiResponse<ManagerResponse[]>>(`${this.url}/users`)
      .pipe(
  ...
  delete(id: number): Observable<ApiResponse<void>> {
    return this.http.delete<ApiResponse<void>>(`${this.url}/users/${id}`).pipe(
      tap((response: ApiResponse<void>) => {
        if (response.success) {
          this.managers.set(
            this.managers().filter((manager) => manager.id !== id)
          );
        }
      })
    );
  ...
  create(
    request: ManagerForCreationDto
  ): Observable<ApiResponse<ManagerResponse>> {
    return this.http
      .post<ApiResponse<ManagerResponse>>(
        `${this.url}/auth/create-manager`,
        request
      )
      .pipe(
        tap((response: ApiResponse<ManagerResponse>) => {
  ...
  createHoliday(
    request: HolidayForCreationDto
  ): Observable<ApiResponse<HolidayResponse>> {
    return this.http
      .post<ApiResponse<HolidayResponse>>(
        `${this.url}/schedule-holidays`,
        request
      )
      .pipe(
  ...
  getHolidays(userId: number): Observable<ApiResponse<HolidayResponse[]>> {
    return this.http
      .get<ApiResponse<HolidayResponse[]>>(
        `${this.url}/schedule-holidays/${userId}`
      )
      .pipe(
        tap((response: ApiResponse<HolidayResponse[]>) => {
          if (response.success && response.data) {
            const parsedData = response.data.map((holiday) => ({
              ...holiday,
              startDate: DateTime.fromISO(holiday.startDate).toFormat(
                'dd/MM/yyyy'
              ),
  ...
  deleteHoliday(holidayId: number): Observable<ApiResponse<void>> {
    return this.http
      .delete<ApiResponse<void>>(`${this.url}/schedule-holidays/${holidayId}`)
      .pipe(
        tap((response: ApiResponse<void>) => {
          if (response.success) {
            this.holidays.set(
              this.holidays().filter((holiday) => holiday.id !== holidayId)
            );
          }
        })
      );
  ...
  resetPassword(payload: ResetPasswordDto): Observable<ApiResponse<void>> {
